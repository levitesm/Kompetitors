/* tslint:disable max-line-length */
import axios from 'axios';
import { format } from 'date-fns';

import * as config from '@/shared/config/config';
import { DATE_FORMAT } from '@/shared/date/filters';
import OfficesService from '@/entities/offices/offices.service';
import { Offices } from '@/shared/model/offices.model';

const mockedAxios: any = axios;
jest.mock('axios', () => ({
  get: jest.fn(),
  post: jest.fn(),
  put: jest.fn(),
  delete: jest.fn()
}));

describe('Service Tests', () => {
  describe('Offices Service', () => {
    let service: OfficesService;
    let elemDefault;
    let currentDate: Date;
    beforeEach(() => {
      service = new OfficesService();
      currentDate = new Date();

      elemDefault = new Offices(0, 'AAAAAAA', '000000', 'AAAAAAA', 'AAAAAAA', 0, 0, 0, 0, 0, null, 0, new Date(), false, 0, 0);
    });

    describe('Service methods', () => {
      it('should find an element', async () => {
        const returnedFromService = Object.assign(
          {
            established: format(currentDate, DATE_FORMAT)
          },
          elemDefault
        );
        mockedAxios.get.mockReturnValue(Promise.resolve({ data: returnedFromService }));

        return service.find(123).then(res => {
          expect(res).toMatchObject(elemDefault);
        });
      });
      it('should create a Offices', async () => {
        const returnedFromService = Object.assign(
          {
            id: 0,
            established: format(currentDate, DATE_FORMAT)
          },
          elemDefault
        );
        const expected = Object.assign(
          {
            established: currentDate
          },
          returnedFromService
        );

        mockedAxios.post.mockReturnValue(Promise.resolve({ data: returnedFromService }));
        return service.create({}).then(res => {
          expect(res).toMatchObject(expected);
        });
      });

      it('should update a Offices', async () => {
        const returnedFromService = Object.assign(
          {
            name: 'BBBBBB',
            address: 'BBBBBB',
            phone: 'BBBBBB',
            post: 'BBBBBB',
            cityAsText: 'BBBBBB',
            numberEmployees: 1,
            numberConsultants: 1,
            numberTechnicals: 1,
            numberHR: 1,
            numberClients: 1,
            established: format(currentDate, DATE_FORMAT),
            isMainOffice: true,
            latitude: 1,
            longitude: 1
          },
          elemDefault
        );

        const expected = Object.assign(
          {
            established: currentDate
          },
          returnedFromService
        );
        mockedAxios.put.mockReturnValue(Promise.resolve({ data: returnedFromService }));

        return service.update(expected).then(res => {
          expect(res).toMatchObject(expected);
        });
      });
      it('should return a list of Offices', async () => {
        const returnedFromService = Object.assign(
          {
            name: 'BBBBBB',
            address: 'BBBBBB',
            phone: 'BBBBBB',
            post: 'BBBBBB',
            cityAsText: 'BBBBBB',
            numberEmployees: 1,
            numberConsultants: 1,
            numberTechnicals: 1,
            numberHR: 1,
            numberClients: 1,
            established: format(currentDate, DATE_FORMAT),
            isMainOffice: true,
            latitude: 1,
            longitude: 1
          },
          elemDefault
        );
        const expected = Object.assign(
          {
            established: currentDate
          },
          returnedFromService
        );
        mockedAxios.get.mockReturnValue(Promise.resolve([returnedFromService]));
        return service.retrieve().then(res => {
          expect(res).toContainEqual(expected);
        });
      });
      it('should delete a Offices', async () => {
        mockedAxios.delete.mockReturnValue(Promise.resolve({ ok: true }));
        return service.delete(123).then(res => {
          expect(res.ok).toBeTruthy();
        });
      });
    });
  });
});
